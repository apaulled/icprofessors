...
def get_faculty(generate_classifier_data=False, use_classifier=False):
    ...
    tracker = []
    for link in links:
        fac = {}  # This is the object I'm generating that you should be able to insert into mongo
                    ...
                    for child in children:
                        if isinstance(child, bs4.element.Tag):
                            ...
                            if child.name == 'p':
                                ...
                                heuristic_header = len(text.strip()) > 0 and text.strip()[-1] == ':'
                                nn_header = False
                                if use_classifier and (not heuristic_header):  # This is a quick hack to minimize calling the model
                                    nn_header = classifier.is_header(text.strip())
                                if heuristic_header or (use_classifier and nn_header):
                                    if use_classifier and nn_header:
                                        print('Found NN Header:', text.strip())
                                ...
                                if len(text_parts) > 1:
                                    ...
                                    if generate_classifier_data and key != 'generic':
                                        paragraphs.extend([part.strip() for part in text_parts])
                                else:
                                    value_text = text.strip()
                                    if value_text != '':
                                        value.append(value_text)
                                        if generate_classifier_data and key != 'generic':
                                            paragraphs.append(value_text)
    ...
    file_name = 'resources/faculty_nn.json' if use_classifier else 'resources/faculty.json'
    with open(file_name, 'w') as f:
        f.write(json.dumps(facs, indent=4))

    if generate_classifier_data:
        with open('resources/data.json', 'w') as f:
            f.write(json.dumps({'headers': headers, 'paragraphs': paragraphs}, indent=4))
